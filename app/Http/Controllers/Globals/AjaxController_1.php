...
//put your code here
    ...
    protected function fn_get_permission($request) {
        ...
        return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'successfully fetching permissions data.', 'valid' => true, 'data' => $permissions]);
    }
    ...
    protected function fn_get_menu_by_level($request) {
        ...
        if ($request->level) {
            $i = $request->level - 1;
            $menu = Tbl_a_menus::get_menu_by_level($i, $request->module_id);
            $strMenu = '';
    ...
    protected function fn_get_menu($request) {
        ...
        if ($request->module_id) {
            $menu_1 = Tbl_a_menus::get_menus($request->module_id, 1, 0);
            if ($menu_1) {
                foreach ($menu_1 AS $keyword => $value) {
                    $menu_2 = Tbl_a_menus::get_menus($request->module_id, ($value->level + 1), $value->id);
                    $arr_menu_2 = [];
                    if ($menu_2) {
                        foreach ($menu_2 AS $key => $val) {
                            $menu_3 = Tbl_a_menus::get_menus($request->module_id, ($val->level + 1), $val->id);
                            $arr_menu_3 = [];
                            if ($menu_3) {
                                foreach ($menu_3 AS $ky => $vl) {
                                    $menu_4 = Tbl_a_menus::get_menus($request->module_id, ($vl->level + 1), $vl->id);
                                    $arr_menu_4 = [];
                                    if ($menu_4) {
                                        foreach ($menu_4 AS $k => $v) {
                                            $menu_5 = Tbl_a_menus::get_menus($request->module_id, ($v->level + 1), $v->id);
                                            $arr_menu_5 = [];
                                            if ($menu_5) {
    ...
    protected function fn_post_group_permission_update($request) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            if ($response) {
                return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'successfully group permission.', 'valid' => true]);
            } else {
                return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'failed update  group permission.', 'valid' => false]);
            }
        }
    }
    ...
    protected function fn_post_add_group_permission($request) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            if ($permissions == '' || empty($permissions) || $permissions == null) {
                ...
                if ($result) {
                    $group_permissions = DB::table('tbl_a_group_permissions AS a')->where('a.permission_id', '=', $result)->where('a.user_id', '=', $this->__user_id)->first();
                    if ($group_permissions == '' || empty($group_permissions) || $group_permissions == null) {
                        ...
                        $result = DB::table('tbl_a_group_permissions')->insert($param_group_permission);
                    }
                ...
                return $this->MyHelper->_set_response('json', ['code' => 500, 'message' => 'Failed insert new permission data exist.', 'valid' => false]);
            }
        }
    }
