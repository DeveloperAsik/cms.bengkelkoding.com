    ...
    public function insert(Request $request) {
        ...
        if (isset($data) && !empty($data)) {
            $params = [
                ...
                'conditions' => [
                    'where' => [
                        ['a.user_id', '=', (int) base64_decode($data['user'])],
                        ['a.group_id', '=', (int) base64_decode($data['group'])]
                    ]
                ]
            ];
            $user_exist = $this->MY_Model->find($request, 'first', $params);
            if (isset($user_exist['data']) && !empty($user_exist['data'])) {
                return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'failed insert data. user already exist or this user group already used.', 'valid' => false]);
            } else {
                $user_detail = [
                    'user_id' => (int) base64_decode($data['user']),
