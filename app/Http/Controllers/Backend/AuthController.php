...
class AuthController extends Controller {
    ...
    public function __construct(Request $request) {
        ...
        $this->Tbl_a_permissions = new Tbl_a_permissions();
    }
    ...
    public function logout(Request $request) {
        $this->Authenticate->clear_session();
    ...
    public function profile(Request $request) {
        ...
        $user = $this->Tbl_a_users->fnGetUserProfiles($request, $this->__user_id);
        $_modal_data = [
            [
        ...
        $logs = $this->MY_Model->find($request, 'all', $param_logs);
    ...
    public function update_profile(Request $request) {
        ...
        $paramUserProfiles = [
            ...
            'join' => [
                'leftJoin' => [
                    ...
                    ['tbl_b_group_permissions as d', 'd.group_id', '=', 'b.group_id'],
                    ['tbl_a_user_profiles as e', 'e.id', '=', 'a.profile_id']
                ]
            ],
            'conditions' => [
                'where' => [
        ...
        $userProfiles = $this->MY_Model->find($request, 'first', $paramUserProfiles);
        $paramGroupPermission = [
            ...
            'join' => [
                'leftJoin' => [
                    ['tbl_a_permissions as b', 'b.id', '=', 'a.permission_id'],
                    ['tbl_a_groups as c', 'c.id', '=', 'a.group_id'],
                    ['tbl_a_modules as d', 'd.id', '=', 'a.module_id']
                ]
            ],
            'conditions' => [
                'where' => [
                    ['a.group_id', '=', $this->__group_id]
                ]
            ]
        ];
        $groupPermissions = $this->MY_Model->find($request, 'all', $paramGroupPermission);
        $user = $this->Converter->array_to_object(['user_profile' => $userProfiles['data'], 'permission' => $groupPermissions['data']]);
        $paramModules = [
            'table_name' => 'tbl_a_modules'
        ];
        $modules = $this->MY_Model->find($request, 'all', $paramModules);
        $paramsGroup = [
            'conditions' => [
        ...
        $permissions = $this->Tbl_a_permissions->get_all_inselect($request, $paramsGroup);
        $_modal_data = [
            [
                'id' => "modal_change_picture",
                'path' => "Public_html.Modals.Extraweb.User.modal_change_picture"
    ...
    public function fnUploadPhoto(Request $request) {
        if (isset($request) && !empty($request)) {
            ...
            if ($result) {
                ...
                return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'successfully upload picture', 'valid' => true, 'data' => ['path' => $path . '/' . $file_name]]);
            } else {
                return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'failed upload picture', 'valid' => false, 'data' => null]);
            }
        }
    }
