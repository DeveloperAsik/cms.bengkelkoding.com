...
class MenuController extends Controller {
    ...
    public function __construct(Request $request, MY_Model $MY_Model, Tbl_a_menus $Tbl_a_menus, Date $Date) {
        parent::__construct($request);
        $this->MY_Model = $MY_Model;
        $this->Tbl_a_menus = $Tbl_a_menus;
        $this->Date = $Date;
    }
            ...
            if (isset($request['a']) && !empty($request['a'])) {
                switch ($request['a']) {
                    ...
                    case 4 :
                        ...
                        break;
                    case 5 :
                        return $this->get_list_tree($request);
                        break;
                }
            } else {
    ...
    public function get_menu_single($request) {
        $menu = [];
        if ($request->id) {
            $menu = $this->Tbl_a_menus->get_menu_by_id($request->id, 1, 0);
        }
        return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'successfully fetching menu data.', 'valid' => true, 'data' => $menu]);
    }
    ...
    public function get_menu_by_level($request) {
        ...
        if ($request->level) {
            $i = $request->level;
            $menu = $this->Tbl_a_menus->get_menu_by_level($request->id);
            $strMenu = '';
        ...
        return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'successfully fetching menu data.', 'valid' => true, 'data' => $strMenu]);
    }

    public function get_list_all($request) {
        $menu = [];
        if ($request->group_id) {
            $menu_1 = $this->Tbl_a_menus->get_menus($request->group_id, 1, 0);
            if ($menu_1) {
                foreach ($menu_1 AS $keyword => $value) {
                    $menu_2 = $this->Tbl_a_menus->get_menus($request->group_id, ($value->level + 1), $value->id);
                    $arr_menu_2 = [];
                    if ($menu_2) {
                        foreach ($menu_2 AS $key => $val) {
                            $menu_3 = $this->Tbl_a_menus->get_menus($request->group_id, ($val->level + 1), $val->id);
                            $arr_menu_3 = [];
                            if ($menu_3) {
                                foreach ($menu_3 AS $ky => $vl) {
                                    $menu_4 = $this->Tbl_a_menus->get_menus($request->group_id, ($vl->level + 1), $vl->id);
                                    $arr_menu_4 = [];
                                    if ($menu_4) {
                                        foreach ($menu_4 AS $k => $v) {
                                            $menu_5 = $this->Tbl_a_menus->get_menus($request->group_id, ($v->level + 1), $v->id);
                                            $arr_menu_5 = [];
                                            if ($menu_5) {
        ...
        return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'successfully fetching menu data.', 'valid' => true, 'data' => $root_menu]);
    }

    public function get_list_tree($request) {
        $menu = [];
        $menu_1 = $this->Tbl_a_menus->get_menus2(1, 0);
        if ($menu_1) {
            foreach ($menu_1 AS $keyword => $value) {
                $menu_2 = $this->Tbl_a_menus->get_menus2(($value->level + 1), $value->id);
                $arr_menu_2 = [];
                if ($menu_2) {
                    foreach ($menu_2 AS $key => $val) {
                        $menu_3 = $this->Tbl_a_menus->get_menus2(($val->level + 1), $val->id);
                        $arr_menu_3 = [];
                        if ($menu_3) {
                            foreach ($menu_3 AS $ky => $vl) {
                                $menu_4 = $this->Tbl_a_menus->get_menus2(($vl->level + 1), $vl->id);
                                $arr_menu_4 = [];
                                if ($menu_4) {
                                    foreach ($menu_4 AS $k => $v) {
                                        $menu_5 = $this->Tbl_a_menus->get_menus2(($v->level + 1), $v->id);
                                        $arr_menu_5 = [];
                                        if ($menu_5) {
                                            foreach ($menu_5 AS $j => $w) {
                                                $arr_menu_5[] = [
    ...
    public function get_menu_by_child($request) {
        ...
        if ($request->parent_id) {
            $menu = $this->Tbl_a_menus->get_menu_by_parent_id($request->module_id, $request->level, $request->parent_id);
        }
        $strMenu = '';
    ...
    public function insert(Request $request) {
        ...
        if (isset($data) && !empty($data)) {
            $parent_menu_id = (int) $data['parent_menu'];
            $parent_menus = $this->Tbl_a_menus->get_by_id($request, $parent_menu_id)['data'];
            $level = 1;
            if ($parent_menus) {
            ...
            $menu_id = 129; //DB::table('tbl_a_menus')->insertGetId($param);
            if ($menu_id) {
                $param_group_permissions = [
                    'table_name' => 'tbl_b_menu_permission',
                    'select' => ['a.group_id','module_id'],
                    'conditions' => [
                        'where' => [
                            ['a.is_active', '=', 1]
                        ]
                ...
                $group_permissions = $this->MY_Model->find($request, 'all', $param_group_permissions);
                if (isset($group_permissions['data']) && !empty($group_permissions['data'])) {
                    $group_permission = [];
                    foreach ($group_permissions['data'] AS $key => $val) {
                        $group_permission[] = [
                            'menu_id' => $menu_id,
                    ...
                    DB::table('tbl_b_menu_permission')->insert($group_permission);
                }
                return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'successfully update modules', 'valid' => true]);
            } else {
                return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'failed update modules.', 'valid' => false]);
            }
        }
    }
            ...
            if (isset($request->a) && !empty($request->a)) {
                ...
                switch ($data['action']) {
                    ...
                    default:
                        ...
                        $parent_menus = $this->Tbl_a_menus->get_by_id($parent_menu_id);
                        $level = 1;
                        if ($parent_menus) {
                            $level = ((int) $parent_menus->level + 1);
                        }
    ...
    public function update_menu(Request $request, $id) {
        ...
        if (isset($data) && !empty($data)) {
            if (isset($data['is_delete']) && !empty($data['is_delete'])) {
                ...
                if ($response) {
                    return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'successfully delete data.', 'valid' => true]);
                } else {
                    return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'failed delete data.', 'valid' => false]);
                }
                ...
                if ($response) {
                    return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'successfully update menu.', 'valid' => true]);
                } else {
                    return $this->MyHelper->_set_response('json', ['code' => 200, 'message' => 'failed update menu.', 'valid' => false]);
                }
            } else {
