...
class DocumentController extends Controller {
    ...
    //put your code here
    protected $MY_Model;
    protected $table_default = 'tbl_a_project_document_files';
    ...
    public function create(Request $request) {
        ...
        $_config = [
            'title_for_header' => 'Create new <b>Document</b> master data management page',
            'create_page' => [
                'title' => 'click to open Document list page',
                'icon' => '<i class="fa-solid fa-list"></i>',
                'link' => config('app.base_extraweb_uri') . '/project/type/view'
            ]
        ];
        $params = [
            'table_name' => 'tbl_a_projects',
        ];
        $projects = $this->MY_Model->find($request, 'list', $params, 'mysql_assessment');
        return view('Public_html.Layouts.Adminlte.dashboard', compact('title_for_layout', '_breadcrumbs', '_config', 'projects'));
    }
    ...
    public function view(Request $request) {
        ...
        $_config = [
            'title_for_header' => '<b>Documents</b> master data management page',
            'create_page' => [
                'title' => 'click to open form create new Documents',
                'icon' => '<i class="fas fa-square-plus"></i>',
                'link' => config('app.base_extraweb_uri') . '/project/documents/create'
            ]
        ];
        return view('Public_html.Layouts.Adminlte.dashboard', compact('title_for_layout', '_breadcrumbs', '_config'));
    }

    public function view_document(Request $request, $id = null) {
        $title_for_layout = config('app.default_variables.title_for_layout');
        $_breadcrumbs = [
            [
                'id' => 1,
    ...
    public function get_list(Request $request) {
        if (isset($request) && !empty($request)) {
            ...
            $params = [
                ...
                'select' => [
                    'a.id', 'a.name AS document_name', 'a.path AS document_path', 'a.project_id',
                    'b.code  AS project_code', 'b.name AS project_name'
                ],
                'conditions' => $conditions,
                'join' => [
                    'leftJoin' => [
                        ['tbl_a_projects AS b', 'b.id', '=', 'a.project_id']
                    ]
                ],
                'order' => [
                    ['a.id', 'asc']
