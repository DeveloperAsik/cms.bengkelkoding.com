...
class PermissionController extends Controller {
    ...
    public function __construct(Request $request, MY_Model $MY_Model, Date $Date) {
        ...
        if (isset($data) && !empty($data)) {
            ...
            if (isset($data['method']) && !empty($data['method'])) {
                foreach ($data['method'] AS $key => $value) {
                    ...
                    $arrParam[] = [
                        'name' => str_replace('//', '/', $data['name'] . '/' . $value . $sufix),
                        'path' => str_replace('//', '/', $data['path'] . '/' . $value . $sufix),
            ...
            if ($arrParam) {
                foreach ($arrParam AS $k => $v) {
                    $permission_id = DB::table($this->table_default)->insertGetId($v);
                    ...
                    if (isset($group_permissions['data']) && !empty($group_permissions['data'])) {
                        ...
                        DB::table('tbl_b_group_permissions')->insert($group_permission);
                    }
                    //tbl_b_user_permissions
                    $param_user_permissions = [
                        'table_name' => 'tbl_b_user_permissions',
                        'select' => ['a.user_id'],
                        'conditions' => [
                            'where' => [
                                ['a.is_active', '=', 1]
                    ...
                    $user_permissions = $this->MY_Model->find($request, 'all', $param_user_permissions);
                    if (isset($user_permissions['data']) && !empty($group_permissions['data'])) {
                        $user_permission = [];
                        foreach ($user_permissions['data'] AS $key => $val) {
                            $group_permission[] = [
                        ...
                        DB::table('tbl_b_user_permissions')->insert($user_permission);
                    }
                    $response = true;
                }
            }
