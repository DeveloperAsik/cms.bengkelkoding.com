    ...
    return {
        ...
        init: function () {
            ...
            var table = $('#user_groups').DataTable({
                ...
                "ajax": {
                    url: _base_extraweb_uri + '/prefferences/user/groups/get_list?a=1',
                    headers: {
            ...
            $('#user_groups').on('click', 'input[type="checkbox"][name="is_active"]', function () {
                var checked = this.checked;
                var id = Base64.encode($(this).attr('data-id'));
                var uri = _base_extraweb_uri + '/prefferences/user/groups/update/' + id;
                var type = 'POST';
                var formdata = {
                    action: 'is_active',
