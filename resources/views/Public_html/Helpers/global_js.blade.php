<script>
    ...
    var fnInitFetchNotif = function(){
        var i = 0;
        setInterval(function(){
            i++;
            if(i%10 == 0){
                var full_url = document.URL;
                var segment = full_url.replace(_base_extraweb_uri ,'');
                switch(segment){
                    case "/messaging/inbox":
                        fnReloadDatatable('#inbox');
                        break;
                    default:
                        fnReloadTotalChat();
                        break;
    ...
    var fnReloadDatatable = function(el){
        $(el).DataTable().ajax.reload();
        return false;
    };
    
    var fnReloadTotalChat = function(){
        if (typeof __is_logged_in !== 'undefined') {
            var uri = _base_extraweb_uri + '/messaging/inbox/get_list?a=2';
            var response = fnAjaxSend({}, uri, 'POST', {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}, false);
            if(response.responseJSON){
                $('span#total_chat').html(response.responseJSON.total_chat);
            }  
        }
        return false;
    };
